...
class HMultiHeadedSelfAttention(nn.Module):
    """ Multi-Headed Dot Product Attention """
    def __init__(self, cfg):
        super().__init__()
        self.proj_q = nn.Linear(cfg.dim, cfg.dim)
        self.proj_k = nn.Linear(cfg.n_heads, cfg.n_heads)
        self.proj_v = nn.Linear(cfg.n_heads, cfg.dim)
        self.proj_h = nn.Linear(cfg.dim, cfg.dim)
        self.drop = nn.Dropout(cfg.p_drop_attn)
        self.scores = None # for visualization
        self.n_heads = cfg.n_heads

    def forward(self, x,hpc, mask):
        #print(x.shape)
        k,v=self.proj_k(hpc),self.proj_v(hpc) #ltm
        q,h=self.proj_q(x),self.proj_h(x) # sequence

        #print(q.shape,k.shape,v.shape,h.shape)
        q, h, k, v = (split_last(x, (self.n_heads, -1)).transpose(1, 2)
                   for x in [q,h, k, v])

        # (B, H, S, W) @ (B, H, W, S) -> (B, H, S, S) -softmax-> (B, H, S, S)
        #print(q.shape,k.shape,v.shape,h.shape)

        scores = q @ (k+h).transpose(-2, -1) / np.sqrt(k.size(-1))
        #scores = q @ (k+h).transpose(-2, -1) / np.sqrt(k.size(-1))
        ...
        # (B, H, S, S) @ (B, H, S, W) -> (B, H, S, W) -trans-> (B, S, H, W)
        h_ = (scores @ v).transpose(1, 2).contiguous()
...
class HBlock(nn.Module):
    def __init__(self,cfg):
        super().__init__()
        self.hattn = HMultiHeadedSelfAttention(cfg)
        self.proj = nn.Linear(cfg.dim, cfg.dim)
...
class HTransformer(nn.Module):
    """ Transformer with Self-Attentive HBlocks"""
    def __init__(self, cfg):
        super().__init__()
        self.embed = Embeddings(cfg)
